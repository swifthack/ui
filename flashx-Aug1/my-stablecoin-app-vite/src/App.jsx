...
const App = () => {
  ...
  useEffect(() => {
    if (currentPersona === 'customer') {
      fetch('/api/bankAdmin/customers', { headers: { 'accept': '*/*' } })
        .then(res => res.json())
        .then(data => {
          const list = Array.isArray(data.data) ? data.data : [];
          console.log("Fetched customer IDs:", list);
          setCustomerIdList(list.map(c => c.email));
        })
  ...
  const renderPage = () => {
    ...
    if (currentPersona === 'customer') {
      console.log("Rendering Customer Dashboard with selectedCustomerId:", selectedCustomerId);
      ...
      switch (activePage) {
        ...
        case 'wallets':
          return <WalletsPage selectedCustomerId={selectedCustomerId} 
            navigateToTransactionsForWallet={navigateToTransactionsForWallet} />;
        case 'payments':
          return <PaymentsPage />;
        case 'transactions':
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans text-gray-900 flex flex-col lg:flex-row">
      ...
      <div className="lg:hidden p-4 bg-white shadow-sm flex justify-between items-center">
        <h1 className="text-xl font-bold text-indigo-700">FlashX</h1>
        {isLoggedIn && (
