...
const DepositModal = ({ isOpen, onClose, wallet, onDeposit }) => {
  ...
  const handleSubmit = (e) => {
    ...
    onDeposit(wallet.id, parseFloat(depositAmount));
    onClose();
    setDepositAmount('');
  };
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-50">
      <div className="bg-white p-8 rounded-lg shadow-xl max-w-md w-full mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="depositAmount" className="block text-sm font-medium text-gray-700 mb-1">Amount ({wallet.currency})</label>
            <input
              type="number"
              id="depositAmount"
              value={depositAmount}
